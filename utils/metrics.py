...
def rank(similarity, q_pids, g_pids, max_rank=10, get_mAP=True):
    if get_mAP:
        indices = torch.argsort(similarity, dim=1, descending=True).to(g_pids.device)
...
def get_alpha(sims):
    ...
    alpha_t2i = evidences + 1
    alpha_i2t = evidences.t() + 1
    return alpha_t2i, alpha_i2t, norm_e

def get_alpha_t(sims):
    evidences = torch.exp(torch.tanh(sims)/0.02)
...
class Evaluator():
    def __init__(self, img_loader, txt_loader):
        ...
        self.logger = logging.getLogger("DCEL.eval")
    ...
    def eval(self, model, i2t_metric=False):
        ...
        sims, _ = torch.topk(
            similarity, k=5, dim=1, largest=True, sorted=True
        )
        ...
        alpha_t2i, alpha_i2t, _ = get_alpha(similarity)
        alpha_t2i_t, alpha_i2t_t, _ = get_alpha_t(similarity)

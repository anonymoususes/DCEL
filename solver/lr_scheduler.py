    ...
    def get_lr(self):
        ...
        if self.last_epoch < 10:
            epoch_ratio = (self.last_epoch - self.warmup_epochs) / (
                    20 - self.warmup_epochs
            )
        #epoch_ratio = (self.last_epoch - self.warmup_epochs) / (
        #self.total_epochs - self.warmup_epochs
        #)
        if self.last_epoch < 15 and self.last_epoch >= 10:
            epoch_ratio = 0.8

        if self.last_epoch < 20 and self.last_epoch >= 15:
            epoch_ratio = 0.9

        if self.last_epoch < 25 and self.last_epoch >= 20:
            epoch_ratio = 0.95

        if self.last_epoch < 35 and self.last_epoch >= 25:
            epoch_ratio = 0.99

        if self.last_epoch < 40 and self.last_epoch >= 35:
            epoch_ratio = 0.995

        if self.last_epoch < 45 and self.last_epoch >= 40:
            epoch_ratio = 0.999

        if self.last_epoch <= 80 and self.last_epoch >= 70:
            epoch_ratio = 0.999
